...
#define ENABLE_VOICE_WAKE_WORD 0
...
static volatile bool voiceInteractionRequested = false;
static volatile bool voiceInteractionInProgress = false;
static volatile bool audioPlaybackInProgress = false;
static unsigned long voiceTriggerCooldownUntil = 0;
static const unsigned long VOICE_TRIGGER_COOLDOWN_MS = 2000;
static esp_sleep_wakeup_cause_t bootWakeupCause = ESP_SLEEP_WAKEUP_UNDEFINED;
static volatile bool ignoreNextButtonRelease = false;
...
// 按钮相关函数
void handleButtonPress(uint32_t pressDurationMs); // 处理按钮按下事件
void enterDeviceDeepSleep();                      // 整机软件关机
void initPowerState();                           // 初始化电源状态
const char *getWakeupCauseName(esp_sleep_wakeup_cause_t cause);
...
void setup()
{
  ...
  {
    ei_printf("  ! WiFi当前未完全连通，下一步将阻塞等待百度Token并持续重试\n");
  }

  // 3. 百度语音令牌初始化
#if GPS_TEST_MODE
  ei_printf("[3/5] GPS测试模式下跳过百度语音令牌获取...\n");
#else
  ei_printf("[3/6] 获取百度语音服务令牌...\n");
  accessToken = waitForAccessToken_baidu();
  ei_printf("  ✓ 百度语音服务初始化成功\n");
#endif

  // 4. 音频系统初始化
#if GPS_TEST_MODE
  ei_printf("[4/5] 初始化功能模块...\n");
  ei_printf("  [GPS TEST MODE] 已启用，仅保留 GPS 主链路，语音/超声波/按钮/光敏任务将被屏蔽。\n");
#else
  ei_printf("[4/6] 初始化功能模块...\n");
  set_i2s();
#endif

  ei_printf("[GPS] 即将执行 gpsInit()...\n");
  gpsInit();
  ei_printf("[GPS] gpsInit() 已返回，准备启动任务...\n");

  // 5. 启动FreeRTOS任务
#if GPS_TEST_MODE
  ei_printf("[5/5] 启动FreeRTOS任务...\n");
#else
  ei_printf("[5/6] 启动FreeRTOS任务...\n");
#endif
  setupTasks();

  // 6. 完成剩余初始化
#if GPS_TEST_MODE
  ei_printf("[5/5] 完成剩余初始化...\n");
#else
  ei_printf("[6/6] 完成剩余初始化...\n");
#endif
#if GPS_TEST_MODE
  ei_printf("  [GPS TEST MODE] 已跳过语音服务和唤醒词初始化。\n");
#endif
  ...
  // 唤醒词推理系统初始化
#if !GPS_TEST_MODE
#if ENABLE_VOICE_WAKE_WORD
  initWakeWordSystem();
#else
  ei_printf("  [语音设置] 已禁用唤醒词，仅支持 GPIO%d 按钮短按唤醒\n", TEST_BUTTON_PIN);
  ei_printf("  [按键设置] 短按按钮唤醒语音，长按按钮软件关机；关机后按按钮2可再次开机\n");
#endif
#endif
}
...
void initPowerState()
{
  ...
  ei_printf("[电源] 本次启动原因: %s\n", getWakeupCauseName(bootWakeupCause));

  if (bootWakeupCause == ESP_SLEEP_WAKEUP_EXT0 && digitalRead(TEST_BUTTON_PIN) == LOW)
  {
    ignoreNextButtonRelease = true;
    ei_printf("[电源] 检测到按钮2唤醒后的持续按下，首次松手将忽略，避免误触发语音\n");
  }
  else
  {
...
void loop()
{
  ...
  if (millis() - lastGpsLogTime >= 5000)
  {
    ...
    if (gpsHasValidFix())
    {
      Serial.printf("[GPS TEST] Latest fix OK | lat=%.6f lon=%.6f utc=%s\n",
                    fix.latitude, fix.longitude, fix.utcTime.c_str());
    }
    else
    {
      Serial.println("[GPS TEST] Waiting for a valid GPS fix...");
    }
  }
  ...
  {
    bool voiceBusy = voiceInteractionRequested || voiceInteractionInProgress || audioPlaybackInProgress;
    ei_printf("[主循环] 系统运行正常，当前仅支持按钮唤醒\n");
    ei_printf("[状态监控] record_status: %s, record_status_me: %s, voice_req: %s, voice_run: %s, playback: %s, wake: %s\n", 
              record_status ? "true" : "false", 
              record_status_me ? "true" : "false",
              voiceInteractionRequested ? "true" : "false",
              voiceInteractionInProgress ? "true" : "false",
              audioPlaybackInProgress ? "true" : "false",
              getWakeupCauseName(bootWakeupCause));
    ...
    if (voiceBusy) {
      if (voiceInteractionStartTime == 0) {
        ...
        voiceInteractionRequested = false;
        voiceInteractionInProgress = false;
...
{
  // 如果语音交互正在进行，跳过唤醒词检测
  if (!record_status || voiceInteractionRequested || voiceInteractionInProgress || audioPlaybackInProgress)
  {
...
{
  if (millis() < voiceTriggerCooldownUntil)
  {
    ei_printf("[唤醒检测] 处于对话结束冷却期，忽略本次唤醒触发\n");
    return;
  }

  if (voiceInteractionRequested || voiceInteractionInProgress || audioPlaybackInProgress)
  {
    ei_printf("[唤醒检测] 当前语音链路忙碌，忽略本次唤醒触发\n");
    return;
  }
  ...
  // 停止后台唤醒词音频采集，避免和正式录音抢占同一麦克风I2S口
  record_status = false;
  ...
  voiceInteractionRequested = true;
...
void createCore0Tasks()
{
  ...
  if (result == pdPASS)
  {
    ei_printf("  [GPS TEST MODE] GPS任务创建成功\n");
  }
  else
  {
    ei_printf("  [GPS TEST MODE] GPS任务创建失败\n");
  }
  return;
#endif
...
{
#if GPS_TEST_MODE
  ei_printf("  [GPS TEST MODE] 已屏蔽核心1任务创建，避免其他模块干扰 GPS 测试。\n");
  return;
#endif
...
void printTaskInfo()
{
  ei_printf("\n--- 任务分配信息 ---\n");
#if GPS_TEST_MODE
  ei_printf("GPS 测试模式任务:\n");
  ei_printf("  - 核心0: GPS定位与上报\n");
  ei_printf("  - 核心1: 已屏蔽\n");
...
void buttonTask(void *parameter)
{
  ...
  while (1)
  {
    ...
    if ((millis() - lastDebounceTime) >= BUTTON_DEBOUNCE_MS && buttonState != stableState)
    {
      ...
      {
        uint32_t pressDurationMs = millis() - pressStartTime;
        if (ignoreNextButtonRelease)
        {
          ignoreNextButtonRelease = false;
          ei_printf("[按钮处理] 已忽略唤醒后的首次松手事件\n");
        }
        else
        {
          handleButtonPress(pressDurationMs);
        }
        pressStartTime = 0;
      }
    }

    vTaskDelay(pdMS_TO_TICKS(BUTTON_TASK_POLL_MS));
  }
}
 ...
 * @brief 处理按钮按下事件
 */
void handleButtonPress(uint32_t pressDurationMs)
{
  ei_printf("[按钮处理] 按键释放，按下时长: %lu ms\n", pressDurationMs);

  if (pressDurationMs >= BUTTON_LONG_PRESS_MS)
  {
    enterDeviceDeepSleep();
    return;
  }

  if (millis() < voiceTriggerCooldownUntil)
  {
    ei_printf("[按钮处理] 当前处于对话结束冷却期，忽略本次按钮触发\n");
    return;
  }

  if (voiceInteractionRequested || voiceInteractionInProgress || audioPlaybackInProgress)
  {
    ei_printf("[按钮处理] 当前语音链路忙碌，忽略本次按钮触发\n");
    return;
  }

  ei_printf("[按钮触发] 短按按钮，启动语音交互\n");

  digitalWrite(LED_BUILT_IN, HIGH); // 点亮LED指示

  // 即使已经关闭唤醒词，这里仍沿用同一套状态机，保持录音流程一致。
  record_status = false;
  record_status_me = false;
...
void enterDeviceDeepSleep()
{
  ei_printf("[电源] 长按按钮，设备准备软件关机\n");
  ...
  if (!esp_sleep_is_valid_wakeup_gpio((gpio_num_t)TEST_BUTTON_PIN))
  {
    ei_printf("[电源] 错误: GPIO%d 不支持深睡唤醒，已取消关机\n", TEST_BUTTON_PIN);
    resetRecordStatus();
    return;
  }

  rtc_gpio_pullup_en((gpio_num_t)TEST_BUTTON_PIN);
  rtc_gpio_pulldown_dis((gpio_num_t)TEST_BUTTON_PIN);

  esp_err_t wakeupErr = esp_sleep_enable_ext0_wakeup((gpio_num_t)TEST_BUTTON_PIN, 0);
  if (wakeupErr != ESP_OK)
  {
    ei_printf("[电源] 错误: 配置按钮唤醒失败: %s\n", esp_err_to_name(wakeupErr));
    resetRecordStatus();
    return;
  }
  ...
  Serial.flush();
  delay(50);
  ei_printf("[电源] 已进入关机待机，按下按钮2可重新开机\n");
  Serial.flush();
...
void voiceTask(void *parameter)
{
  ...
  {
    // 检查是否有语音交互触发（唤醒词或按钮）
    if (voiceInteractionRequested && !voiceInteractionInProgress)
    {
      ei_printf("[语音任务] 检测到语音交互触发信号\n");
      voiceInteractionRequested = false;
      voiceInteractionInProgress = true;
...
void handleVoiceInteraction()
{
  ...
  // 单次对话模式：只有整轮识别和播报都结束后，才恢复待唤醒状态
  voiceTriggerCooldownUntil = millis() + VOICE_TRIGGER_COOLDOWN_MS;
  ei_printf("[语音交互] 单次对话完成，进入冷却期 %lu ms\n", VOICE_TRIGGER_COOLDOWN_MS);
  resetRecordStatus();
...
void updateNavigationStatus()
{
  ...
  if (voiceInteractionRequested || voiceInteractionInProgress || audioPlaybackInProgress)
  {
    ei_printf("[导航] 当前语音链路忙碌，跳过本次导航播报更新\n");
    return;
  }
  ...
  HTTPClient http;
  http.begin(String(SERVER_BASE_URL) + "/navigation_update");
...
void stopNavigation()
{
  ...
  // 通知后端退出导航
  HTTPClient http;
  http.begin(String(SERVER_BASE_URL) + "/exit_navigation");
  http.addHeader("Content-Type", "application/json");
